...
######################################################################
#        Name: enumerator.sh
# Description: Enumerator runs enum4linux on machines that have port 139 or 445 open
# Output: *_enum_*.txt
#   ASCII-ART: http://patorjk.com/software/taag/#p=display&f=ANSI%20Shadow&t=%20%20RED%20TEAM%0AEnumerator
######################################################################
...
grep ' 139/open' *Info.gnmap | cut -d " " -f 2 | sort | uniq > _ENUMT
grep ' 145/open' *Info.gnmap | cut -d " " -f 2 | sort | uniq >> _ENUMT

cat _ENUMT | sort | uniq > _ENUM
rm _ENUMT 2>/dev/null
for name in $(cat _ENUM); do
    setterm -cursor off
    echo -ne "${GC}[+]${NC} Enumerating $name..."    
    /usr/bin/enum4linux -l -v -n -a $name > ../${PROJECT}_enum_${name}.txt 2>&1
    echo "Done."
    echo -e "${GC}[+]${NC} TXT report generated: ${GC}${PROJECT}/${PROJECT}_enum_${name}.txt${NC}"    
done
rm _ENUM

echo -e "${GC}[+]${NC} R3D 734M Enumerator is complete."

...
######################################################################
#        Name: peeker.sh
# Description: Peeker does a quick nmap scan to get you started by looking at ports 21,22,23,25,80,443,139,445,389,5555,10000.
# Output: output/*_PeekInfo.gnmap
#   ASCII-ART: http://patorjk.com/software/taag/#p=display&f=ANSI%20Shadow&t=RED%20TEAM%0A%20%20PEEKER
######################################################################
...
echo -e "${GC}[+]${NC} nmap -e $INT --stats-every 4s -sS -Pn -p 21,22,23,25,80,443,139,445,389,5555,10000 -vvv -O -A --min-rate 800 -T4 -oA "$PROJECT"_nmap_PeekInfo -iL ${PROJECT}_hosts_Up.txt"
echo -e "${GC}[*]${NC} Peeking at Live hosts..."

nmap -e $INT --stats-every 4s -sS -Pn -p 21,22,23,25,80,443,139,445,389,5555,10000 -vvv -O -A --min-rate 800 -T4 -oA "$PROJECT"_nmap_PeekInfo -iL ${PROJECT}_hosts_Up.txt &>/dev/null &

sleep 5
until [ "$STATUS" = "success" ]; do
    PERCENT=`cat "$PROJECT"_nmap_PeekInfo.xml 2>/dev/null |grep "percent" |cut -d '"' -f 6 |tail -1 |cut -d "." -f 1`
    STATUS=`cat "$PROJECT"_nmap_PeekInfo.xml 2>/dev/null |grep -i "exit" |cut -d '"' -f 10`
...
/usr/bin/xsltproc ${PROJECT}_nmap_PeekInfo.xml -o ../${PROJECT}_PeekInfo.html

echo -e "${GC}[+]${NC} HTML report generated: ${GC}${PROJECT}/${PROJECT}_PeekInfo.html${NC}"
echo -e "${GC}[+]${NC} R3D 734M Peeker is complete." 
echo ""
